...
class Multi(Formula):
    def __init__(self, *args):
      if len(args) > 0 and isinstance(args[0], (tuple, list)):
        self.terms = frozenset(makeFormula(x) for x in args[0][0])
      else:
    ...
    def seekNegations(self):
        this = self.getClass()
        dual = self.getDualClass()()
        terms = [x.terms.seekNegations() if isinstance(x, this) else x for x in self.terms]
        for x in terms:
            temp = terms.copy()
            temp.remove(x)
            if isinstance(x, (Variable, Not)) and (Not(x) in temp or  x.x in temp):
                return dual
        return self
    ...
    def simplify(self):
        ...
        if len(terms) == 1:
            return terms[0]
        return self.getClass()(*terms).flatten().seekNegations()
