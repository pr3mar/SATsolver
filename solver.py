...
def SATsolver(formula, valuations, add=None):
    ...
    if [] in formula: # if there is an empty clause in the formula, the formula is not satisfied
        return None
    for literal, value in valuations.items():
        if value is None: # try assigning a True value to the picked literal
            calc_valuations = SATsolver(list(formula), dict(valuations), [literal])
...
formula, vars = parse_inst('inputs/input2')

solution = SATsolver(formula, vars)
